...
def get_settings_by_group(db: Session, group: str):
    ...
    return (
        ...
        .filter(
            SystemSettingModel.group == group, SystemSettingModel.is_hidden == False
        )
        .order_by(SystemSettingModel.sort_order)
        .all()
    )
...
def init_default_settings(db: Session):
    ...
    default_settings = [
        ...
        # --- IP-защита: авто-бан при переборе паролей ---
        {
            ...
            "description": "Включить бан IP и авто-бан при переборе паролей",
            "sort_order": 10,
        },
        {
            "key": "ip_autoban_threshold",
            "value": "10",
            "value_type": "int",
            "group": "security",
            "label": "Порог авто-бана (неудач)",
            "description": "Сколько неудачных входов с IP в окне приводит к бану",
            "sort_order": 11,
        },
        {
            "key": "ip_autoban_window_minutes",
            "value": "15",
            "value_type": "int",
            "group": "security",
            "label": "Окно подсчёта неудач (мин)",
            "description": "За какой период считаются неудачные входы для авто-бана",
            "sort_order": 12,
        },
        {
            "key": "ip_ban_minutes",
            ...
            "description": "На сколько минут блокируется IP при авто-бане",
            "sort_order": 13,
        },
        # --- IP-защита: ограничение всплесков запросов (DDoS) ---
        {
            "key": "ddos_protection_enabled",
            "value": "true",
            "value_type": "bool",
            "group": "security",
            "label": "Защита от всплесков (DDoS)",
        ...
        },
        {
            ...
            "description": "Макс. запросов с одного IP в окне до блокировки",
            "sort_order": 15,
        },
        {
            "key": "ddos_window_seconds",
            "value": "60",
            "value_type": "int",
            "group": "security",
            "label": "Окно лимита (сек)",
            "description": "Интервал подсчёта запросов для DDoS-защиты",
            "sort_order": 16,
        },
        {
            "key": "ddos_ban_minutes",
            "value": "15",
            "value_type": "int",
            "group": "security",
            "label": "Бан при DDoS (мин)",
            "description": "На сколько минут блокируется IP при превышении лимита запросов",
            "sort_order": 17,
        },
        # === Интерфейс ===
        {
    ...
    for setting_data in default_settings:
        existing = (
            db.query(SystemSettingModel)
            .filter(SystemSettingModel.key == setting_data["key"])
            .first()
        )
    ...
    migration_setting = (
        db.query(SystemSettingModel)
        .filter(SystemSettingModel.key == "permissions_migrated_2026_01")
        .first()
    )
    if migration_setting and migration_setting.value != "true":
        for permission in ("edit_tasks", "delete_tasks"):
            ...
            if perm:
                ...
                db.add(
                    RolePermissionModel(
                        role="worker", permission=permission, is_allowed=False
                    )
                )
