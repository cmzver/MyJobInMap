...
def get_sla_metrics(
    ...
    completed_tasks = base_query.filter(TaskModel.status == TaskStatus.DONE.value).count()
    in_progress_tasks = base_query.filter(TaskModel.status == TaskStatus.IN_PROGRESS.value).count()
    cancelled_tasks = base_query.filter(TaskModel.status == TaskStatus.CANCELLED.value).count()
    ...
    if completion_hours_list:
        ...
        if len(completion_hours_list) % 2 == 0 and len(completion_hours_list) > 1:
            median_hours = (completion_hours_list[mid - 1] + completion_hours_list[mid]) / 2
        else:
    ...
    for priority_key in ["PLANNED", "CURRENT", "URGENT", "EMERGENCY"]:
        rank_key = {"PLANNED": "1", "CURRENT": "2", "URGENT": "3", "EMERGENCY": "4"}[priority_key]
        ...
        sla_rate = (priority_in_sla / priority_completed * 100) if priority_completed > 0 else 0
