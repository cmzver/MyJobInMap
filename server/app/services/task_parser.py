...
class ParsedTask:
    ...
    )  # 1=Плановая, 2=Текущая, 3=Срочная, 4=Аварийная
...
def parse_dispatcher_format(text: str) -> Optional[ParsedTask]:
    ...
    name_match = re.search(
        r"([А-ЯЁ][а-яё]+\s+[А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+)?)\s*$", text
    )
    if name_match:
        potential_name = name_match.group(1)
        # Исключаем известные не-имена
        if not re.match(
            r"(Лен\s+обл|Санкт\s+Петербург)", potential_name, re.IGNORECASE
        ):
...
def parse_standard_format(text: str) -> Optional[ParsedTask]:
    ...
    if len(lines) == 1:
        return ParsedTask(title="Новая заявка", address=lines[0], description=lines[0])
...
def parse_dispatcher_message(text: str) -> dict:
    ...
    try:
        ...
        return {"success": False, "error": "Не удалось распознать формат сообщения"}
