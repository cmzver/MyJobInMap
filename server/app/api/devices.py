    ...
    for device in devices:
        ...
        result.append(
            DeviceResponse(
                ...
                last_active=(
                    device.last_active.isoformat() if device.last_active else ""
                ),
            )
        )
...
async def register_device(
    ...
    logger.info(
        "Device registration: token=%s... name=%s",
        device.token[:30],
        device.device_name,
    )

    existing = (
        db.query(DeviceModel).filter(DeviceModel.fcm_token == device.token).first()
    )
    ...
    new_device = DeviceModel(
        user_id=user.id, fcm_token=device.token, device_name=device.device_name
    )
    db.add(new_device)
    db.commit()
...
async def list_devices_info(
    ...
    return {"count": count, "firebase_enabled": firebase_app is not None}
