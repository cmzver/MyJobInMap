...
async def get_finance_stats(
    ...
    query = tenant.apply(db.query(TaskModel), TaskModel).filter(
...
async def get_workers_stats(
    ...
    task_query = tenant.apply(
        db.query(
            TaskModel.assigned_user_id,
            func.count(TaskModel.id).label("total_tasks"),
            func.sum(case((TaskModel.status == "DONE", 1), else_=0)).label(
                "completed_tasks"
            ),
            func.sum(case((TaskModel.status == "IN_PROGRESS", 1), else_=0)).label(
                "in_progress_tasks"
            ),
            func.sum(
                case(
                    (
                        TaskModel.status == "DONE",
                        case((TaskModel.is_paid == True, 1), else_=0),
                    ),
                    else_=0,
                )
            ).label("paid_tasks"),
            func.sum(
                case(
                    (
                        TaskModel.status == "DONE",
                        case((TaskModel.is_remote == True, 1), else_=0),
                    ),
                    else_=0,
                )
            ).label("remote_tasks"),
            func.sum(
                case(
                    (
                        ...
                        case(
                            (
                                TaskModel.is_paid == True,
                                func.coalesce(TaskModel.payment_amount, 0.0),
                            ),
                            else_=0,
                        ),
                    ),
        ...
        TaskModel,
    ).filter(TaskModel.assigned_user_id.isnot(None))
