...
from app.models import (
    UserModel, DeviceModel, TaskModel,
    CustomFieldModel, RolePermissionModel,
    get_db,
)
...
async def admin_update_task(
    ...
    task = tenant.apply(db.query(TaskModel), TaskModel).filter(TaskModel.id == task_id).first()
    if not task:
    ...
    if "assigned_user_id" in update_data:
        if update_data["assigned_user_id"] is None:
            ...
            assigned_user = db.query(UserModel).filter(UserModel.id == update_data["assigned_user_id"]).first()
            if not assigned_user:
                raise HTTPException(status_code=404, detail="User not found")
            tenant.enforce_access(assigned_user, detail="Cannot assign user from another organization")
...
async def get_devices(
    ...
    tenant = TenantFilter(admin)
    devices_query = db.query(DeviceModel).join(UserModel, DeviceModel.user_id == UserModel.id)
    if not tenant.is_superadmin:
        devices_query = devices_query.filter(UserModel.organization_id == admin.organization_id)
