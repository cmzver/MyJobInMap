...
def main():
    parser = argparse.ArgumentParser(description="Резервное копирование базы данных FieldWorker")
    parser.add_argument("--keep", type=int, default=7, help="Хранить бэкапы N дней (default: 7)")
    parser.add_argument("--output", type=str, help="Директория для бэкапов")
    parser.add_argument("--with-photos", action="store_true", help="Включить фото в бэкап")
    ...
    try:
        ...
        if args.with_photos:
            ...
            if photos_backup:
                ...
                print(f"✅ Бэкап фото создан: {photos_backup.name} ({photos_size:.1f} MB)")
