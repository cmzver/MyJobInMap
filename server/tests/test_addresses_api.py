...
class TestAutocompleteEndpoints:
    ...
    def _seed_addresses(self, client: TestClient, auth_headers: dict):
        ...
        addresses = [
            {"address": "Москва, ул. Ленина, 1", "city": "Москва",
             "street": "ул. Ленина", "building": "1", "entrance_count": 4},
            {"address": "Москва, ул. Ленина, 2", "city": "Москва",
             "street": "ул. Ленина", "building": "2", "entrance_count": 3},
            {"address": "Москва, Тверская, 10", "city": "Москва",
             "street": "Тверская", "building": "10"},
            {"address": "СПб, Невский, 5", "city": "Санкт-Петербург",
             "street": "Невский проспект", "building": "5"},
        ]
    ...
    def test_autocomplete_cities(self, client: TestClient, auth_headers: dict):
        ...
        response = client.get("/api/addresses/autocomplete/cities", headers=auth_headers)
        assert response.status_code == 200
    ...
    def test_autocomplete_cities_with_query(self, client: TestClient, auth_headers: dict):
        """Filter cities by query parameter."""
        response = client.get(
            "/api/addresses/autocomplete/cities?q=Моск",
            headers=auth_headers
        )
        assert response.status_code == 200
        cities = response.json()
    ...
    def test_autocomplete_streets(self, client: TestClient, auth_headers: dict):
        ...
        response = client.get("/api/addresses/autocomplete/streets", headers=auth_headers)
        assert response.status_code == 200
        streets = response.json()
        assert isinstance(streets, list)
        assert len(streets) >= 2  # ул. Ленина + Тверская + Невский

    def test_autocomplete_streets_filtered_by_city(self, client: TestClient, auth_headers: dict):
        """Streets filtered by city."""
        response = client.get(
            "/api/addresses/autocomplete/streets?city=Москва",
            headers=auth_headers
        )
        assert response.status_code == 200
        streets = response.json()
    ...
    def test_autocomplete_entrance_no_match(self, client: TestClient, auth_headers: dict):
    ...
    def test_autocomplete_full(self, client: TestClient, auth_headers: dict):
        ...
        response = client.get(
            "/api/addresses/autocomplete/full?q=Ленина",
            headers=auth_headers
        )
        assert response.status_code == 200
        results = response.json()
...
class TestAddressFindByComponents:
    ...
    def test_find_by_components(self, client: TestClient, auth_headers: dict):
        ...
        client.post(
            "/api/addresses",
            json={"address": "Москва, проспект Мира, 99",
                  "city": "Москва", "street": "проспект Мира", "building": "99"},
            headers=auth_headers
        )
        response = client.get(
...
class TestAddressDeactivate:
    ...
    def test_deactivate_address(self, client: TestClient, auth_headers: dict):
        ...
        response = client.post(
            f"/api/addresses/{addr_id}/deactivate",
            headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_deactivate_not_found(self, client: TestClient, auth_headers: dict):
        response = client.post(
            "/api/addresses/99999/deactivate",
            headers=auth_headers
        )
        assert response.status_code == 404
...
class TestAddressFilters:
    ...
    def test_search_addresses_param(self, client: TestClient, auth_headers: dict):
        client.post("/api/addresses",
                    ...
                    headers=auth_headers)
        response = client.get(
            "/api/addresses?search=УникальнаяСтрока123",
            headers=auth_headers
        )
        assert response.status_code == 200
