...
class TestSystemSettingModel:
    ...
    def test_get_typed_value_string(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="string_setting", value="hello", value_type="string", label="String"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_get_typed_value_bool_true(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="bool_setting", value="true", value_type="bool", label="Bool"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_get_typed_value_bool_false(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="bool_false", value="false", value_type="bool", label="Bool False"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_get_typed_value_int(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="int_setting", value="42", value_type="int", label="Int"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_get_typed_value_float(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="float_setting", value="3.14", value_type="float", label="Float"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_get_typed_value_none(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="none_setting", value=None, value_type="string", label="None"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_set_typed_value_bool(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="set_bool", value_type="bool", label="Set Bool"
        )
    ...
    def test_set_typed_value_json(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="set_json", value_type="json", label="Set JSON"
        )
...
class TestSettingFunctions:
    ...
    def test_set_setting_existing(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="to_update", value="old_value", value_type="string", label="To Update"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_get_settings_by_group(self, db_session: Session):
        ...
        other = SystemSettingModel(
            key="other_group_setting", value="x", group="other_group", label="Other"
        )
    ...
    def test_get_settings_by_group_excludes_hidden(self, db_session: Session):
        ...
        visible = SystemSettingModel(
            key="visible", value="v", group="mixed", label="Visible", is_hidden=False
        )
        hidden = SystemSettingModel(
            key="hidden", value="h", group="mixed", label="Hidden", is_hidden=True
        )
    ...
    def test_get_all_settings(self, db_session: Session):
        ...
        for i in range(5):
            setting = SystemSettingModel(
                key=f"all_test_{i}", value=str(i), group="all", label=f"All {i}"
            )
            db_session.add(setting)
        db_session.commit()
...
class TestRolePermissionModel:
    ...
    def test_create_permission(self, db_session: Session):
        ...
        permission = RolePermissionModel(
            role="admin", permission="view_dashboard", is_allowed=True
        )
    ...
    def test_permission_denied(self, db_session: Session):
        ...
        permission = RolePermissionModel(
            role="worker", permission="delete_tasks", is_allowed=False
        )
        db_session.add(permission)
        db_session.commit()
    ...
    def test_query_permissions_by_role(self, db_session: Session):
        ...
        permissions = [
            RolePermissionModel(
                role="dispatcher", permission="view_tasks", is_allowed=True
            ),
            RolePermissionModel(
                role="dispatcher", permission="edit_tasks", is_allowed=True
            ),
            RolePermissionModel(
                role="dispatcher", permission="delete_tasks", is_allowed=False
            ),
        ...
        dispatcher_perms = (
            db_session.query(RolePermissionModel)
            .filter(RolePermissionModel.role == "dispatcher")
            .all()
        )
...
class TestSettingInvalidTypes:
    ...
    def test_invalid_int_returns_zero(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="bad_int", value="not a number", value_type="int", label="Bad Int"
        )
        db_session.add(setting)
        db_session.commit()
    ...
    def test_invalid_float_returns_zero(self, db_session: Session):
        ...
        setting = SystemSettingModel(
            key="bad_float", value="not a float", value_type="float", label="Bad Float"
        )
        db_session.add(setting)
        db_session.commit()
