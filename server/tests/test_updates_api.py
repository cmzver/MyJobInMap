...
def _upload_apk(client, auth_headers, version_name="1.1.0", version_code=2,
                release_notes="Bug fixes", is_mandatory=False, apk_data=None,
...
class TestUploadApk:
    ...
    def test_upload_requires_admin(self, client, client_with_dispatcher):
        ...
        response = client_with_dispatcher.post(
            "/api/updates/upload",
            files={"file": ("app.apk", _create_fake_apk(), "application/vnd.android.package-archive")},
    ...
    def test_upload_duplicate_version_code_rejected(self, client, auth_headers):
        ...
        response = _upload_apk(client, auth_headers, version_code=5, version_name="1.2.0")
    ...
    def test_upload_lower_than_latest_version_code_rejected(self, client, auth_headers):
        ...
        response = _upload_apk(client, auth_headers, version_name="2.1.0", version_code=9)
        assert response.status_code == 409
