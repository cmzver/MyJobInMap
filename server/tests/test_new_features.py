...
class TestSlaApi:
    ...
    def test_sla_is_tenant_scoped(self, client, db_session):
        ...
        db_session.add_all([org1, org2, admin, worker1, worker2, own_task, foreign_task])
        db_session.commit()

        login = client.post("/api/auth/login", data={"username": "sla_admin", "password": "secret123"})
...
class TestWebSocketManager:
    ...
    def test_broadcast_scoped_by_organization(self, manager):
        ...
        run_async(manager.connect(ws3, user_id=3, organization_id=None, is_superadmin=True))
...
class TestWebSocketEndpoint:
    ...
    def test_ws_refresh_token_rejected(self, client, admin_user):
        ...
        refresh_token = create_refresh_token({"sub": admin_user.username, "user_id": admin_user.id})
