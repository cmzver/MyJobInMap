...
class TestAddressSystemsCRUD:
    ...
    def test_create_system(
        ...
        response = client.post(
            f"/api/addresses/{address.id}/systems", json=data, headers=auth_headers
        )
    ...
    def test_get_systems(
        ...
        # Создаём системы
        system1 = AddressSystemModel(
            address_id=address.id,
            system_type="video_surveillance",
            name="Видео",
            status="active",
        )
        system2 = AddressSystemModel(
            address_id=address.id,
            system_type="intercom",
            name="Домофон",
            status="active",
        ...
        response = client.get(
            f"/api/addresses/{address.id}/systems", headers=auth_headers
        )
    ...
    def test_delete_system(
        ...
        """Удаление системы."""
        system = AddressSystemModel(
            address_id=address.id, system_type="fire_alarm", name="ОПС", status="active"
        )
        db_session.add(system)
        db_session.commit()
        db_session.refresh(system)
        ...
        response = client.delete(
            f"/api/addresses/{address.id}/systems/{system.id}", headers=auth_headers
        )
        assert response.status_code == 204

        # Проверяем, что удалена
        check = (
            db_session.query(AddressSystemModel)
            .filter(AddressSystemModel.id == system.id)
            .first()
        )
...
class TestAddressEquipmentCRUD:
    ...
    def test_create_equipment(
        self, client: TestClient, address_with_system, auth_headers: dict
    ):
        ...
        response = client.post(
            f"/api/addresses/{address.id}/equipment", json=data, headers=auth_headers
        )
    ...
    def test_get_equipment_filtered_by_system(
        self,
        client: TestClient,
        address_with_system,
        db_session: Session,
        ...
        # Все
        response = client.get(
            f"/api/addresses/{address.id}/equipment", headers=auth_headers
        )
        assert len(response.json()) == 2

        # Только по системе
        response = client.get(
            f"/api/addresses/{address.id}/equipment?system_id={system.id}",
            headers=auth_headers,
        )
...
class TestAddressContactsCRUD:
    ...
    def test_create_contact(
        ...
        response = client.post(
            f"/api/addresses/{address.id}/contacts", json=data, headers=auth_headers
        )
...
class TestAddressHistory:
    ...
    def test_history_created_on_system_add(
        self, client: TestClient, db_session: Session, auth_headers: dict
    ):
        ...
        # Проверяем историю
        response = client.get(
            f"/api/addresses/{address.id}/history", headers=auth_headers
        )
        assert response.status_code == 200
