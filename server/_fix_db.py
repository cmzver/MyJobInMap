...
if 'organizations' not in tables:
    ...
    ''')
    cur.execute('CREATE INDEX IF NOT EXISTS ix_organizations_name ON organizations(name)')
    cur.execute('CREATE INDEX IF NOT EXISTS ix_organizations_slug ON organizations(slug)')
    print('  Done!')
else:
...
if 'organization_id' not in task_cols:
    ...
    cur.execute('ALTER TABLE tasks ADD COLUMN organization_id INTEGER REFERENCES organizations(id)')
    print('  Done!')
else:
...
if 'organization_id' not in addr_cols:
    ...
    cur.execute('ALTER TABLE addresses ADD COLUMN organization_id INTEGER REFERENCES organizations(id)')
    print('  Done!')
else:
